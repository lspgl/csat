...
class Calibrator:
    ...
    def f(self, c, x, y, w=1):
        Ri = self.calc_R(x, y, *c)
        delta = Ri - Ri.mean()
        delta_w = delta * w
        return delta_w

    def leastsq_circle(self, x, y, w=1):
        x_m = np.mean(x)
        y_m = np.mean(y)
        center_estimate = x_m, y_m
        center, ier = optimize.leastsq(self.f, center_estimate, args=(x, y, w))
        xc, yc = center
    ...
    def computeAll(self, tofile=True):
        self.calibration = [self.computeMidpoint(fn) for fn in self.fns]
        if tofile:
    ...
    def plotCalibration(self):
        ...
        mr = np.mean(radii)
        weights = [1 - np.exp(-np.abs(mr - r)) for r in radii]
        ...
        for i, mp in enumerate(mps):
            circle = plt.Circle((mp[0], mp[1]), np.abs(mr - radii[i]) / 5, lw=1, alpha=0.3)
            ax.add_artist(circle)
